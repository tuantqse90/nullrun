...
- 2026-07-12 — **Rewards = hệ sinh thái Tasco/VETC** (trọng tâm track). Catalog thiết kế bằng **workflow đa-agent** (research Tasco thật + 8 agent theo nhóm + tổng hợp cân bằng giá): migration 0012 seed 32 quà — `vetc` (nạp Ví, vé tháng/năm), `vetcgo` (thuê xe tự lái/có tài xế, đưa đón sân bay, voucher đặt xe), `tasco` (xăng/EV, trạm dừng nghỉ, Carpla chăm xe, e-Parking, T-money, bảo hiểm, merch). ~10 điểm/1.000đ, 60–9500 điểm. Adapter `TascoMock` (mã VETC-/VGO-/TASCO-); **cổng link Guardian giờ chỉ áp cho quà guardian** (Tasco/VETC đổi thẳng bằng điểm). iOS `RewardsView`: chia **section theo đối tác** (badge màu thương hiệu). **Artwork vẽ tay bằng SwiftUI** (`RewardArt` — cổng thu phí, xe hơi, trụ xăng, ly cà phê, bánh răng+cờ lê, biển P, áo, khiên…) thay icon phẳng, gradient theo brand, badge mệnh giá voucher. **Review đối kháng (1 agent)** tìm 2 lỗi thật đã sửa: (1) màn voucher đưa thu ngân hardcode "Guardian" → thêm `partner` vào redemption response, brand theo đối tác; (2) reconciliation + partner_stats gộp chung mọi đối tác → tách theo partner. Verified prod: đổi VETC ra `partner:vetc` + mã `VETC-...`. Đã deploy.
